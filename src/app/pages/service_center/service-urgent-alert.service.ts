...
export class ServiceBookingUrgentAlertService {
    private serviceBookingRequest$ = new BehaviorSubject<any | null>(null);
    ...
    private async initializeService() {
        if (isPlatformBrowser(this.platformId)) {
            ...
            this.listenForServiceUrgentRequests();
        }
    }
    ...
    private listenForServiceUrgentRequests() {
        if (!this.serviceCenterId) {
            console.error('Service Center ID not available. Cannot listen for service urgent requests.');
            return;
        }
        ...
        const serviceBookingsRef = collection(this.firestore, 'service_bookings');
        ...
        onSnapshot(serviceBookingsQuery, async (snapshot) => {
            for (const change of snapshot.docChanges()) {
                if (change.type === 'added') {
                    ...
                    if (this.isNewRequest(requestTime)) {
                        console.log('New urgent service booking request detected:', requestId);
                        ...
                        this.ngZone.run(() => {
                            this.serviceBookingRequest$.next(enhancedData);
                            this.lastRequestTime = new Date();
                        });
                    }
                }
            ...
            console.error('Error listening to service booking urgent requests:', error);
        });
    }
    ...
    getServiceBookingRequestsStream() {
        return this.serviceBookingRequest$.asObservable();
    }
