 ...
 private fs = inject(Firestore);
  ...
  async ngOnInit() {
    ...
    for (const [i, status] of ['pending', 'confirmed', 'in_progress'].entries()) {
      const q1 = query(bkCol, where('workshopId', '==', workshopId), where('status', '==', status));
      const s = await getDocs(q1);
      this.cards[i].value = s.size;
    }
    const sevenDaysAgo = Timestamp.fromDate(new Date(Date.now() - 7*24*3600*1000));
    const q4 = query(bkCol, where('workshopId', '==', workshopId), where('status', '==', 'completed'), where('updatedAt', '>=', sevenDaysAgo));
    ...
    const all = await getDocs(query(bkCol, where('workshopId','==',workshopId)));
